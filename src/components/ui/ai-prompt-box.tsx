...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => (
  <textarea
    className={cn(
      "flex w-full rounded-md border-none bg-transparent px-4 py-3 text-lg text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[56px] resize-none",
      className
    )}
    ref={ref}
    rows={1}
...
// Dialog primitives (for image preview)
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-[90vw] md:max-w-[800px] translate-x-[-50%] translate-y-[-50%] gap-4 border border-border bg-popover p-0 shadow-xl rounded-2xl",
        className
      )}
      {...props}
    >
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-3xl border border-border bg-card p-3 shadow-[0_8px_30px_rgba(0,0,0,0.10)] transition-all duration-300",
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const handleDrop = React.useCallback((e: React.DragEvent) => {
    ...
    const imgs = dropped.filter((file) => isImageFile(file));
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn("w-full transition-all duration-300 ease-in-out", isRecording && "border-destructive/70", className)}
                ...
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 rounded-full bg-background/70 p-0.5 transition-opacity"
                    >
          ...
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            <PromptInputAction tooltip="Upload image">
              <button
                ...
                className="flex h-9 w-9 text-muted-foreground cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-muted hover:text-foreground"
            ...
            <div className="flex items-center">
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1.5 px-2.5 py-1 border h-9",
                  showSearch ? "bg-primary/15 border-primary text-primary" : "bg-transparent border-transparent text-muted-foreground hover:text-foreground"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1.5 px-2.5 py-1 border h-9",
                  showThink ? "bg-primary/15 border-primary text-primary" : "bg-transparent border-transparent text-muted-foreground hover:text-foreground"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1.5 px-2.5 py-1 border h-9",
                  showCanvas ? "bg-primary/15 border-primary text-primary" : "bg-transparent border-transparent text-muted-foreground hover:text-foreground"
                )}
              >
