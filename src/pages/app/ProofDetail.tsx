...
import {
  ArrowLeft, Lock, ShieldCheck, Copy, RefreshCw, Ban, Share2, Sparkles,
  CheckCircle2, AlertCircle, X as XIcon, Hourglass, Clock, FileCode2, Cpu, Eye,
} from "lucide-react";
import {
  issuedProofs, zkTemplates, type IssuedProof, type ProofStatus,
...
const STATUS_META: Record<ProofStatus, { label: string; className: string; icon: typeof ShieldCheck }> = {
  ...
  revoked: { label: "Revoked", className: "border-destructive/30 bg-destructive/10 text-destructive", icon: XIcon },
...
const ProofDetail = () => {
  ...
  const [busy, setBusy] = useState<null | "generate" | "renew" | "revoke">(null);
      ...
      <>
        ...
        <div className="flex-1 px-4 md:px-8 pb-8">
          <Card className="rounded-2xl border-border/60 border-dashed p-12 text-center">
            <p className="text-muted-foreground mb-4">No proof exists with id <code>{id}</code>.</p>
  ...
  const onRenew = () => {
    ...
    setTimeout(() => {
      ...
      setBusy(null);
      toast.success("Proof renewed", { description: "Lifetime extended by 30 days." });
    }, 900);
  };
  ...
  const headerTitle = proof ? proof.template : `Generate: ${seedTemplate.title}`;
      ...
      <div className="flex-1 px-4 md:px-8 pb-8 space-y-6">
            ...
            <div className="flex items-start gap-4 flex-wrap">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {proof && meta && (
                    <Badge variant="outline" className={"rounded-full text-xs " + meta.className}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {proof ? (
                  <>
                    ...
                    >
                      <RefreshCw className={"h-4 w-4 mr-1.5 " + (busy === "renew" ? "animate-spin" : "")} />
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Revoke this proof?</AlertDialogTitle>
                          <AlertDialogDescription>
                            Verifiers will reject <code className="font-mono">{proof.id}</code> immediately.
                            You can always generate a new one from the same template.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                  ...
                  >
                    <Sparkles className={"h-4 w-4 mr-1.5 " + (busy === "generate" ? "animate-pulse" : "")} />
                    {busy === "generate" ? "Generating proof…" : "Generate ZK proof"}
                  </Button>
                )}
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card className="rounded-2xl border-border/60 p-6 space-y-5">
            <div>
              <h3 className="font-display font-semibold text-foreground">Predicate</h3>
            ...
            <div>
              <Label className="text-xs text-muted-foreground uppercase tracking-wide">Statement</Label>
              <Input
              ...
              {!proof && (
                <p className="text-[11px] text-muted-foreground mt-1.5">
                  Hint: use comparators like <code>&gt;</code>, <code>&lt;</code>, <code>==</code>.
                </p>
              )}
            </div>

            <div>
              <Label className="text-xs text-muted-foreground uppercase tracking-wide">What gets disclosed</Label>
              <div className="mt-1.5 p-4 rounded-xl border border-border/60 bg-muted/30 text-sm space-y-2">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span>Underlying score is <span className="font-medium text-foreground">never revealed</span></span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Lock className="h-4 w-4 shrink-0" />
          ...
          <Card className="rounded-2xl border-border/60 p-6 space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-display font-semibold text-foreground">Proof artifact</h3>
                <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {!proof ? (
              ...
              <>
                <div>
                  <Label className="text-xs text-muted-foreground uppercase tracking-wide">Proof hash</Label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                    ...
                    <p className="text-sm font-medium text-foreground mt-1">{proof.zkSystem}</p>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wide flex items-center gap-1">
                      <FileCode2 className="h-3 w-3" /> Size
                    </p>
                    <p className="text-sm font-medium text-foreground mt-1">{proof.size}</p>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wide flex items-center gap-1">
                      <Eye className="h-3 w-3" /> Verifications
                    </p>
                    <p className="text-sm font-medium text-foreground mt-1">{proof.verifications}</p>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wide flex items-center gap-1">
                    ...
                    <p className={"text-sm font-medium mt-1 " +
                      (proof.status === "expired" ? "text-warning" : "text-foreground")}>
                      {proof.expiresAt}
                    </p>
                  </div>
                </div>
        ...
        {proof && (
          <Card className="rounded-2xl border-border/60 p-6">
            <h3 className="font-display font-semibold text-foreground mb-4">Activity</h3>
            <div className="space-y-3">
              {[
                { icon: ShieldCheck, label: "Proof issued",       sub: proof.issuedAt,                tone: "text-success" },
                ...(proof.consumer ? [{
                  icon: Eye, label: `Verified by ${proof.consumer}`, sub: `${proof.verifications} times`, tone: "text-muted-foreground" as const,
                }] : []),
                ...(proof.status === "revoked" ? [{
                  icon: Ban, label: "Proof revoked", sub: "Just now", tone: "text-destructive" as const,
                }] : []),
                ...(proof.status === "expired" ? [{
                  icon: AlertCircle, label: "Proof expired", sub: proof.expiresAt, tone: "text-warning" as const,
                }] : []),
              ].map((event, i) => {
